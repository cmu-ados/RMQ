...
int zmq::rdma_address_t::to_string (std::string &addr_) const
{
    ...
    if (_address.family () == AF_INET6) {
        std::stringstream s;
        s << "rdma://[" << hbuf << "]:" << ntohs (_address.ipv6.sin6_port);
        addr_ = s.str ();
    } else {
        std::stringstream s;
        s << "rdma://" << hbuf << ":" << ntohs (_address.ipv4.sin_port);
        addr_ = s.str ();
    }
    return 0;
}
...
zmq::rdma_address_mask_t::rdma_address_mask_t () : _address_mask (-1)
{
    memset (&_network_address, 0, sizeof (_network_address));
}

int zmq::rdma_address_mask_t::mask () const
{
    return _address_mask;
}

int zmq::rdma_address_mask_t::resolve (const char *name_, bool ipv6_)
{
...
int zmq::rdma_address_mask_t::to_string (std::string &addr_) const
{
...
bool zmq::rdma_address_mask_t::match_address (const struct sockaddr *ss_,
